-- ============================================
-- ADD PROPERTY TYPES FILTERING TO match_saved_searches()
-- ============================================
-- 
-- Updates match_saved_searches() to support property type filtering.
-- If criteria->'propertyTypes' is null or empty, do not filter.
-- Else require listings.property_type to be in the array.
-- ============================================

-- Recreate function with property types support
...
BEGIN
  ...
  WITH matched_listings AS (
    ...
    WHERE 
      ...
      -- Property types filter: if criteria->'propertyTypes' is null or empty, do not filter
      -- Else require listings.property_type to be in the array
      AND (
        -- No property types filter (null or empty array) - match all types
        (ss.criteria->'propertyTypes' IS NULL 
         OR jsonb_array_length(COALESCE(ss.criteria->'propertyTypes', '[]'::jsonb)) = 0)
        OR
        -- Property types filter exists and is non-empty - require listing.property_type to be in array
        (l.property_type IS NOT NULL 
         AND l.property_type = ANY(
           SELECT jsonb_array_elements_text(ss.criteria->'propertyTypes')
         ))
      )
  )
...
COMMENT ON FUNCTION public.match_saved_searches() IS 
  'Matches active saved searches against live listings based on criteria.location_keyword, criteria.buy_box filters (beds, baths, price), criteria.propertyTypes (array of property types), and area mode (any/radius/polygon). If criteria.propertyTypes is null or empty, matches all property types. Otherwise requires listings.property_type to be in the array. Radius mode uses Haversine distance calculation with listings.latitude and listings.longitude columns. Requires at least one filter to be present (location_keyword, buy_box, or area) to prevent matching all listings. Returns statistics about matches created, searches processed, and listings checked. Idempotent: uses ON CONFLICT to prevent duplicate matches.';
